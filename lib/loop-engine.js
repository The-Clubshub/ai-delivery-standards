...
const CATEGORY_KEYWORDS = {
  ...
  model_routing: ["model routing", "ai provider", "ai_provider", "gpt-5.5", "glm-5.2", "premium review", "requires_premium_review"],
...
function initLoop(argv, runtime) {
  ...
  const tasks = planTasks(specSummary, standards, verificationCommands);
  const state = createLoopState({
    ...
    builderCommand: options["builder-command"] || "",
    reviewerCommand: options["reviewer-command"] || ""
  });
...
function planTasks(specSummary, standards, verificationCommands) {
  const tasks = [];
  const addTask = (title, sourceItems, options = {}) => {
    ...
    const aiProvider = options.ai_provider || routeAiProvider(title, sourceItems, standardsCategories, options.risk || "medium");
    tasks.push({
      id,
      title,
      status: "pending",
      ...
      ai_reviewer: routeAiReviewer(aiProvider),
...
function routeAiProvider(title, sourceItems, standardsCategories, risk) {
  ...
  if (premiumTriggers.some((item) => item.category === "auth or permissions" || item.category === "billing or payments" || item.category === "security")) {
    return {
      provider: "openai",
      model: "gpt-5.5",
      reason: "high-risk security or business-critical code",
      requires_premium_review: true
    };
  }
  if (premiumTriggers.some((item) => item.category === "architecture") || /\b(strategy|plan|planning|context)\b/i.test(title)) {
    return {
      provider: "openai",
      model: "gpt-5.5",
      reason: "high-level reasoning and structural decisions",
      requires_premium_review: true
    };
  }
  ...
  if (/\b(document|docs|readme|copy)\b/i.test(text)) {
    return {
      provider: "zai",
      model: "glm-5.2",
...
function routeAiReviewer(aiProvider) {
  if (!aiProvider) return null;
  if (aiProvider.model === "glm-5.2" || aiProvider.requires_premium_review) {
    return {
      provider: "openai",
      model: "gpt-5.5",
      reason: aiProvider.requires_premium_review ? "premium review required" : "final review equal or stronger than implementation model"
    };
  }
  return null;
}
...
function validateTaskModelRouting(task) {
  ...
  const validProviders = new Set(["openai", "zai", "anthropic", "other"]);
  if (aiProvider.provider && !validProviders.has(aiProvider.provider)) {
    findings.push(finding("blocker", `${task.id} uses unsupported provider ${aiProvider.provider}.`, "Use openai, zai, anthropic, or other with an explicit reason."));
  }

  const validModels = new Set(["gpt-5.5", "glm-5.2", "other"]);
  if (aiProvider.model && !validModels.has(aiProvider.model)) {
    findings.push(finding("blocker", `${task.id} uses unsupported model ${aiProvider.model}.`, "Use gpt-5.5, glm-5.2, or other with an explicit reason."));
  }
  ...
  if (premiumTriggers.length > 0 && aiProvider.requires_premium_review !== true) {
    findings.push(finding(
      ...
      "Set requires_premium_review: true and route final review to GPT-5.5."
    ));
  }

  const reviewer = task.ai_reviewer;
  if ((aiProvider.model === "glm-5.2" || aiProvider.requires_premium_review) && (!reviewer || reviewer.model !== "gpt-5.5")) {
    findings.push(finding(
      "blocker",
      `${task.id} requires GPT-5.5 review for ${aiProvider.model} or premium-review work.`,
      "Record ai_reviewer.model as gpt-5.5 before merge."
    ));
  }

  if (reviewer && !reviewModelIsAtLeastImplementation(reviewer.model, aiProvider.model)) {
    findings.push(finding(
      "blocker",
      `${task.id} final review model ${reviewer.model} is weaker than implementation model ${aiProvider.model}.`,
      "Use an equal-or-stronger final review model."
    ));
  }
  ...
  if (aiProvider.model === "other" || aiProvider.provider === "other") {
    findings.push(finding(
      "important",
      `${task.id} uses an 'other' provider or model.`,
      "Confirm the plan declares relative model strength before relying on it."
    ));
  }
...
function reviewModelIsAtLeastImplementation(reviewModel, implementationModel) {
  if (reviewModel === implementationModel) return true;
  const reviewStrength = MODEL_STRENGTH[reviewModel] || 0;
  const implementationStrength = MODEL_STRENGTH[implementationModel] || 0;
...
function runExternalAgentCommand(commandTemplate, state, task, loopDir, role, runtime) {
  const promptPath = path.join(loopDir, LOOP_FILE_NAMES.builderPrompt);
  const command = commandTemplate
    .replaceAll("{prompt}", shellQuote(promptPath))
    .replaceAll("{loopDir}", shellQuote(loopDir))
    .replaceAll("{taskId}", shellQuote(task.id))
    .replaceAll("{target}", shellQuote(state.target));
  const safety = commandSafety(command, state.autonomyTier);
  if (!safety.allowed) {
    return {
      ok: false,
...
function renderTasksMarkdown(state) {
  ...
  lines.push("", "## AI Model Routing", "", "| Task | Provider | Model | Reason | Premium Review | Reviewer |", "| --- | --- | --- | --- | --- | --- |");
  for (const task of state.tasks) {
    const aiProvider = task.ai_provider || {};
    const reviewer = task.ai_reviewer ? `${task.ai_reviewer.provider} ${task.ai_reviewer.model}` : "N/A";
    lines.push(`| ${task.id} | ${escapeTable(aiProvider.provider || "")} | ${escapeTable(aiProvider.model || "")} | ${escapeTable(aiProvider.reason || "")} | ${aiProvider.requires_premium_review === true ? "true" : "false"} | ${escapeTable(reviewer)} |`);
  }
...
Options:
  ...
  --builder-command <command> External builder hook with {prompt}, {loopDir}, {taskId}, {target}.
  --reviewer-command <command> Reserved external reviewer hook.
